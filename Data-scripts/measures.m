...
eps = 0.00125;                                                              % Noise amplitude

a = 3.83;                                                                  % Logistic parameter

depth = 14;                                                                % We use 2^depth boxes in total
n = 10;                                                                    % Number of particles in a box
noise = 750;                                                               % Noise realisations considered

f = @(x,omega)(a*x(:,1).*(1-x(:,1)) + omega(:));                           % Logistic function

Inexp = @IN_exp_sure;
Incon = @IN_con_compl_sure;                                                % There exists w st third iterate is contracting.

% Other choices of compartments include:

% Inexp = @IN_exp_compl_sure;
% Incon = @IN_con_sure;

% Inexp = @IN_exp_two_three_sure;                                          %for all w, it must expand in three iterates OR in two it must expand. This ensures far away enough from attractor
% Incon = @IN_con_compl_two_three_sure;
...
Pt = tpmatrix1d(tree, f, B, depth, 1, noise, eps);                         % Trans prob matrix
...
[xh_cond_right,lambda_cond_right] = eigs(sparse(transpose(Pt_cond)),...
    1,'LR');                                                               % Right ev for em
ev_cond_right = abs(xh_cond_right(:,1))/norm(xh_cond_right(:,1),1);        % norm
[xh_cond_right2,lambda_cond_right2] = eigs(sparse(transpose(Pt_cond2)),...
    1,'LR');
...
fprintf('Expected escape time from Expanding: %.4f \n',...
    escape_t);
fprintf('CLE from Expanding: %.4f \n',...
    Lyap_cond);
fprintf('\n');

fprintf('Expected escape time from Contracting: %.4f \n',...
    escape_t2);
fprintf('CLE from Contracting: %.4f \n',...
    Lyap_cond2);
fprintf('\n');
...
fprintf('Lyapunov Exponent: %.4f \n',...
    Lyap);
fprintf('\n');
fprintf('Weighted Lyapunov Exponent: %.4f \n',...
    L_weighted);
fprintf('\n');
...
function IN_con_compl_two_three_sure = IN_con_compl_two_three_sure(x, eps)
a = 3.83;
f = @(x,omega)(a*x(:,1).*(1-x(:,1)) + omega);
aux3 = ((abs(a*(1-2.*x).*a.*(1-2.*f(x, -eps)).*a.*(1-2.*f(f(x, -eps), +eps))) < 1)...
    +(abs(a*(1-2.*x).*a.*(1-2.*f(x, +eps)).*a.*(1-2.*f(f(x, +eps), -eps))) < 1)...
...
aux2 = ((abs(a*(1-2.*x).*a.*(1-2.*f(x, +eps))) < 1)...
    +(abs(a*(1-2.*x).*a.*(1-2.*f(x, -eps)))<1));
aux = aux3 + aux2;
IN_con_compl_two_three_sure = 1 - (aux < 1);
end


%Surely contracting on third or second iterate (this is the same as above)
